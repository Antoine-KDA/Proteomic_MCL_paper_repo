...
save_dir <- paste0("./Results/Non_Filtered_Only_", gsub(" ", "_", format(Sys.Date(), "%Y %m %d")))
...
All_results_NonFiltered_LOOCV <- list()
...
for(t in names(trainlist)){
  ...
  save(list = to_export, file = file.path(save_dir, paste0("RFrfe_NonFiltered_", 
                                                           paste0(export_name, "_results_parallel_LOOCV"), 
                                                           "_station.Rdata")))
  
  All_results_NonFiltered_LOOCV[[paste0(export_name, "_results_parallel_LOOCV")]] <- results_parallel
...
# Save the results
save(All_results_NonFiltered_LOOCV, file = file.path(save_dir, paste0("RFrfe_ll_results_NonFiltered_LOOCV_list.Rdata")))
...
names(All_results_NonFiltered_LOOCV) <- gsub("_results_parallel_LOOCV", "", names(All_results_NonFiltered_LOOCV))
...
Final_Model_list_NonFiltered <- list()

# Run over the loop
for(running_name in names(All_results_NonFiltered_LOOCV)){
  
  
  
  training_data <- trainlist[[running_name]]
  testing_data <- testlist[[running_name]]
  model_to_take <- All_results_NonFiltered_LOOCV[[running_name]]
  
  message("\n\nThe current running model is: ", names(All_results_NonFiltered_LOOCV[running_name]), "\n",
  ...
  # Save the plot
  dev.copy(png, 
           file = file.path(save_dir, paste0(running_name, "_", "NonFiltered", "_Predictors_in_the_final_model.png")), 
           units = "cm", 
           res = 300, 
           height = 25, 
           width = 12)
  ...
  # Export the results
  rio::export(best_subsets_df, file.path(save_dir, paste0(running_name, "_", "NonFiltered", "_Resamples_Predictors.xlsx")))
  rio::export(best_variables, file.path(save_dir, paste0(running_name, "_", "NonFiltered", "_Best_Predictors.xlsx")))
  ...
  Final_Model_list_NonFiltered[[paste0(running_name, "_Final_model_for_prediction")]] <- BestPredModel
  ...
  # Save the plot
  dev.copy(png, 
           file = file.path(save_dir, paste0(running_name, "_", "NonFiltered", "_Variable_Importance_in_the_final_model.png")), 
           units = "cm", 
           res = 300, 
           height = 25, 
           width = 12)
  ...
  # Save the plot
  dev.copy(png, 
           file = file.path(save_dir, paste0(running_name, "_", "NonFiltered", "_ROC_Curve_on_test.png")), 
           units = "cm", 
           res = 300, 
           height = 25, 
           width = 12)
  ...
  rio::export(perfexport_test, file = file.path(save_dir, paste0(running_name, "_", "NonFiltered", 
                                                                 "_perfomance_on_test.xlsx")))
  
  
  rm(list = (setdiff(ls(), c("All_results_NonFiltered_LOOCV", "trainlist", "testlist", "save_dir", "Final_Model_list_NonFiltered", "start_time"))))
  
  
}

# Save the final models for predictions
save(Final_Model_list_NonFiltered, file = file.path(save_dir, "Final_Model_list_NonFiltered.Rdata"))
