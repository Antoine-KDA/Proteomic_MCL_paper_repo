...
for(s in 4:6){
  ...
  results_list <- list()
   ...
   for(i in names(trainlist)){
    traindata = trainlist[[i]]
    sizes = floor(2:(ncol(traindata)/nsize))# Changed the size here from floor(2:(ncol(traindata)/3)) to floor(2:(ncol(traindata)/6)) for a small set of variables
    # Filtered models
  ...
  # Save the model
  treeBagRFE_results_list <- results_list
  save(treeBagRFE_results_list, rfe_ctrl, trainlist, file = file.path(save_dir, paste0("treeBagRFE_Filtered_NF_Size", nsize, "_station.Rdata")))
  
  # Transform - and Get the best tuning parameters in all models ----------------
  
  # load(file = file.path(save_dir, "treeBagRFE_Filtered_NF_station.Rdata"))
  # results_list <- treeBagRFE_results_list; rm(treeBagRFE_results_list, rfe_ctrl, trainlist)
  ...
  VarPlot
  ggsave(VarPlot, file = file.path(save_dir, paste0("treeBagRFE_Filtered_NF_Size", nsize, "_Variables.png")), units = "cm", 
         height = 20, width = 15)
  ...
  png(file = file.path(save_dir, paste0("treeBagRFE_Filtered_NF_Size", nsize, "_MaxRocTrain.png")), units = "cm", 
  ...
  rio::export(PerfBestModel_on_train, file = file.path(save_dir, paste0("treeBagRFE_Filtered_NF_Size", nsize, "_PerfBestModel_on_train.xlsx")))
  ...
  modAuc_test <- GetAuc(Actual_class = testlist$All$POD24, Pred_class = pred_class_test$pred)
  modAuc_test
  ...
  roc_curve <- roc(testlist$All$POD24, pred_numeric)
  ...
  png(file = file.path(save_dir, paste0("treeBagRFE_Filtered_NF_Size", nsize, "_ROC_Curve_on_test.png")), units = "cm", 
  ...
  rio::export(perfexport_test, file = file.path(save_dir, paste0("treeBagRFE_Filtered_NF_Size", nsize, "_perfomance_on_test.xlsx")))
  rio::export(bestpredictors, file = file.path(save_dir, paste0("treeBagRFE_Filtered_NF_Size", nsize, "_bestpredictors.xlsx")))
  ...
  rm(Accuracy_test, auc_value, modAuc_test, n, namtoplot, pred_numeric, 
     table_grob, roc_curve, treeBagRFE_results_list, results_list, 
     pred_class_test, MaxRocTrain, dta, dattoplot, confmatTest, cfmatBestMod,
     bestpredictors, bestmodel, perfexport_test, testlist)
